...
//-------------------------------------------------------------------------------------------
     ...
     * @return
     */
    public long insertJsonSQL(String TabName, JSONObject Tab) {
        long ret = -1;
        // SELECT count(*) FROM sqlite_master WHERE type='table' AND name='table_name';
        try {
            SQLLiteORM dbHelper = new SQLLiteORM(context);
    ...
    public long updateJsonSQL(String TabName, JSONObject Tab) {
        int ret = -1;
        // SELECT count(*) FROM sqlite_master WHERE type='table' AND name='table_name';
        try {
            SQLLiteORM dbHelper = new SQLLiteORM(context);
    ...
    public boolean getIsExistTab(String TabName) {
        ...
        SQLLiteORM dbHelper = new SQLLiteORM(context);
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        Cursor cursor = db.rawQuery("SELECT name FROM sqlite_master WHERE name=\"" + TabName + "\" LIMIT 1", null);
    ...
    public JSONArray getJsonArray(String TabName, String where) {
        SQLLiteORM dbHelper = new SQLLiteORM(context);
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        String whereLoc="";
        if (where.length()>0) {
            whereLoc = " where "+where;
        }
        Cursor cursor = db.rawQuery("select * from " + TabName + whereLoc, null);
        final JSONArray tmp = getHashTabFromCursorEny(cursor);
        cursor.close();
        dbHelper.close();
        // Log.d("MainActivity",tmp.toString());
