    ...
    def show(self):
        ...
        if not self.parentElement.attrib.get("multiselect") == None:
            ...
            <tr cmptype=\"ComboItem\" isd3repeaterSkip="true">
                 <td>
                     <input type=\"checkbox\" cont=\"allstate\" onclick=\"D3Api.ComboBoxCtrl.setStateAll(D3Api.getControlByDom(this,'ComboBox'),this.checked)\" />
                     <span class=\"btnOC\" comboboxname=\"{self.parentName}\"></span>
                     <span>Все</span>
                 </td>
            </tr>
            """)
            self.print(f"""
            <tr cmptype="ComboItem" isd3repeater="true" id="d3ctrl{self.genName()}" name="{self.name}" value="{self.value}" comboboxname="{self.parentName}" {atr}  {eventsStr}>
                <td>
            ...
            self.print(f"""
            <tr cmptype="ComboItem"  name="cmp{self.name}" comboboxname="{self.parentName}" value="{self.value}" id="d3ctrl{self.genName()}" {atr} {eventsStr}>
              <td>
